...
void ofSoundMixer::Ping(int source, float duration) {
    if (source < 0 || source >= sourceProperties.size()) {
        std::cerr << "Invalid source ID!" << std::endl;
        return;
    }
    mutex.lock();
    sourceProperties[source].volume = 1.f;
    mutex.unlock();
}

void ofSoundMixer::Play(int source) {
    if (source < 0 || source >= sourceProperties.size()) {
        std::cerr << "Invalid source ID!" << std::endl;
        return;
    }
    mutex.lock();
    sourceProperties[source].volume = ofLerp(sourceProperties[source].volume, 1, 0.8);
    mutex.unlock();
}
...
void ofSoundMixer::Stop(int source) {
    ...
    mutex.lock();
    sourceProperties[source].volume = ofLerp(sourceProperties[source].volume, 0, 0.5);
    mutex.unlock();
}
